...
print "AUTO REGISTER, KLAIM VOUCHER, GOPAY SENDER V3\n";
print "Thanks to : mr.Yusfik Helmi\n\n";
echo "[+] Masukan 62 untuk INDOG asw dan 1 untuk US panteq\n";
echo "[+] Nomor lu anjeng: ";
$nope = trim(fgets(STDIN));
$register = register($nope);
if ($register == false)
	{
	echo "[!] Gagal Mendapatkan OTP asuu!\n";
	}
  else
	{
	echo "[+] OTP nya ASU masukin: ";
	$otp = trim(fgets(STDIN));
	$verif = verif($otp, $register);
	if ($verif == false)
		{
		echo "[+] Gagal Mendaftar Ya anjeng, sabar!\n";
		}
	  else
		{
		
		echo "[+] Mencoba klaim  ya asw, makanya sabar goblok :v\n";
		$claim = claim($verif);
		if ($claim == false)
			{
			echo "[!] Gagal Klaim anjeng, coba lagi\n";
			}
		  else
			{
			echo $claim . "\n";
	...
	$headers[] = 'User-uuid: 081388771465';
